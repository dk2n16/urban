...
from mosaic_reclassify_binary import MosaicRasters, ReclassifyBinary
...
class MosaicReclassifyTest(unittest.TestCase):
	...
	def setUp(self):
		...
		self.reclassify = ReclassifyBinary('dataout/2015/urban_2015_1_0_ND.tif')
	...
	# 	self.assertTrue(os.path.exists(tiff_path)) 	FOR SOME REASON GDAL CAN'T FIND THE VRT IN THE UNITTEST BUT IT FINDS IT WHEN RUNNING THE SCRIPT PROPERLY

	def test_reclassify_object_instantiated(self):
		self.assertEqual('dataout/{0}/urban_{0}_1_0_ND.tif'.format(self.mosaic_class_true.year), self.reclassify.path_to_raster)

	def test_raster_reclassified(self):
		self.assertTrue(os.path.exists('dataout/{0}/urban_{0}_1_ND.tif'.format(self.mosaic_class_true.year)))
